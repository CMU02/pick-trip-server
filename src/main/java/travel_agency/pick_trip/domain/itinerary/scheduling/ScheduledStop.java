 ...
 * @param elevationGainMeters   이전 스톱에서 이 스톱까지 구간의 상승고도(m). 하루 첫 스톱·자동차 구간·고도 미상은 0
 * @param inclinePenaltyMinutes 같은 구간에서 오르막으로 추가된 이동시간(분). 하루 첫 스톱·자동차 구간·고도 미상은 0
 */
public record ScheduledStop(
    ...
    /** 경사 정보가 없는 스톱(스케줄링 폴백 등). */
    public ScheduledStop(String contentId, String title, int order, String reason,
                         LocalTime startTime, LocalTime endTime, List<String> notes, boolean autoRest) {
        this(contentId, title, order, reason, startTime, endTime, notes, autoRest, 0.0, 0);
    }

    /** 일반 스톱(자동 삽입 아님, 경사 정보 없음). 호출부 대부분이 이 형태라 축약 생성자를 둔다. */
    public ScheduledStop(String contentId, String title, int order, String reason,
