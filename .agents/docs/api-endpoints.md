...
| `dayStartTimes` | `"HH:mm"` 문자열 배열 | 없음(전부 09:00) | 일차별 하루 시작 시각. 인덱스가 일차 순서(1일차 = `[0]`)이고, 원소가 `null`이거나 일차 수보다 배열이 짧으면 그 일차는 기본값(09:00)으로 시작한다. 배열이 일차 수보다 길면 남는 원소는 무시한다. 05:00~18:00 범위를 벗어나거나 형식이 올바르지 않으면 `VALIDATION_FAILED` |
...
각 항목(`items[]`)에는 **이전 스톱 → 이 스톱** 구간의 값이 따로 실린다. 프론트가 "오르막 반영 +N분" 캡션을 붙이는 용도다.

- `elevationGainMeters` — 구간 상승고도(m). 하루 첫 스톱·자동차 구간·고도 미상은 `0`.
- `inclinePenaltyMinutes` — 오르막으로 추가된 이동시간(분). 일차 `totalTravelMinutes` 에 이미 포함돼 있으므로 더하지 않는다.
- 저장·수정 요청(`POST /api/v1/itineraries`, `PATCH /api/v1/itineraries/{itineraryId}`)의 항목에 같은 이름으로 실어 보내면 저장되고, 저장된 일정 조회 응답에도
  같은 필드로 내려온다. 생략하거나 값이 없던 기존 일정은 `0` 이다.
